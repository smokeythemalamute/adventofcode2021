...
class Grid:
    ...
    def get(self, x: int, y: int, default_value=0) -> int:
        if x < 0 or x >= self.w:
    ...
    def set(self, x: int, y: int, value: int):
        if x < 0 or x >= self.w:
            raise ValueError(f"x out of bounds, x={x}")
        if y < 0 or y >= self.h:
            raise ValueError(f"y out of bounds, y={y}")
        self.data[y][x] = value

    def try_set(self, x: int, y: int, value: int):
        try:
            self.set(x, y, value)
        except ValueError:
            pass
    ...
    def map(self, fn: Callable[[int, int, Grid]]):
        new_grid = self.copy()
        for x, y in product(range(new_grid.w), range(new_grid.h)):
            fn(x, y, new_grid)
        return new_grid

    def all(self, fn: Callable[[int, int, Grid], bool]):
        return all([fn(x, y, self) for x, y in product(range(self.w), range(self.h))])
...
class Step:
    ...
    def __call__(self, startx: int, starty: int, grid: Grid):
        points_to_process = [(startx, starty)]
        while len(points_to_process) > 0:
            x, y = points_to_process[0]
            val = grid.get(x, y)
            if (x, y) in self.flashed:
                pass  # Flashed this pass
            elif val == 9:
                self.flashed += [(x, y)]
                points_to_process += [
                    (x - 1, y - 1),
                ...
                grid.try_set(x, y, grid.get(x, y) + 1)
            points_to_process = points_to_process[1:]


def load_grid(filename: str) -> Grid:
    with open(filename) as f:
        return Grid([[int(c) for c in l.strip()] for l in f.readlines()])


if __name__ == "__main__":
    for filename in ["dec11_test.txt", "dec11_data.txt"]:
        grid = load_grid(filename)
        flashes = 0
        for i in range(100):
            step = Step()
            grid = grid.map(step)
            flashes += len(step.flashed)

        print(filename, ": 100 steps : flashes =", flashes)

        grid = load_grid(filename)
        step = 0
        while not grid.all(lambda x, y, g: g.get(x, y) == 0):
            grid = grid.map(Step())
            step += 1

        print(filename, ":", step, "steps : all flashed")
