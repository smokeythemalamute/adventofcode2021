...
def fn(input: List[int]) -> int:
    return [1 if i > 0 and input[i] > input[i - 1] else 0 for i in range(len(input))]


def conv(input: List[int], window: List[int]) -> List[int]:
    return [
        sum([input[i + j] * weight for j, weight in enumerate(window)])
        for i in range(len(input) - len(window) + 1)
    ]
    # or numpy.convolve(input, window, "valid")
