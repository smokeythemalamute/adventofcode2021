...
# lines). In this example, the first fold instruction is fold along
# y=7, which designates the line formed by all of the positions where
...
# above, 17 dots are visible - dots that end up overlapping after
...
class Paper:
    def __init__(self, points: List[Tuple[int, int]]) -> None:
        ...
        self.w = max([y for x, y in points])
        self.h = max([x for x, y in points])

    def fold(self, axis: str, offset: int) -> Paper:
        new_points = []
        for x, y in self.points:
            if axis == "x":
                if x > offset:
                    delta = abs(x - offset)
                    new_points += [(offset - delta, y)]
                else:
                    new_points += [(x, y)]
            elif axis == "y":
                if y > offset:
                    delta = abs(y - offset)
                    new_points += [(x, offset - delta)]
                else:
                    new_points += [(x, y)]
            else:
    ...
    def __str__(self) -> str:
        return "\n".join(
            [
                "".join(
                    ["#" if (x, y) in self.points else "." for x in range(self.h + 1)]
                )
...
def parse_file(filename: str) -> Tuple[List[Tuple[int, int]], List[Tuple[str, int]]]:
    points = []
    instructions = []
    ...
    section = "points"
    for line in lines:
        # Change sections on an empty line
        if len(line.strip()) == 0:
            if section == "points":
                section = "instructions"
        else:
            if section == "points":
                x, y = [int(x) for x in point_re.match(line).groups()]
                points += [(x, y)]

            if section == "instructions":
                axis, offset = fold_re.match(line).groups()
                instructions += [(axis, int(offset))]

    return points, instructions


if __name__ == "__main__":
    for filename in ["dec13_test.txt", "dec13_data.txt"]:
        points, instructions = parse_file(filename)
        paper = Paper(points)
        for i, inst in enumerate(instructions):
            axis, offset = inst
            paper = paper.fold(axis, offset)
            if i == 0:
                print(
                    filename, ":", len(paper.points), "points visible after fold", i + 1
                )
        print(filename, ":")
        print(paper)
